...
sum_of_weights_actual <- read.csv('weight_map_HaufeAAB/General_PB1_vertex_ABS_sum_Haufe_weights.csv')
...
write.csv(SOW_p_values, file = paste0(dirout,"Vertex_sum_of_weights_p_vals.csv")) #save out SOW p values

SOW_p_values_FDR <- p.adjust(SOW_p_values,"fdr") #FDR corrected p-vals
write.csv(SOW_p_values_FDR, file = paste0(dirout,"Vertex_sum_of_weights_p_vals_FDR.csv")) #save out SOW p FDR values 
