...
# observed weight map
weights_act_A = np.load("Haufe_PRS_040724/Weight_Haufe_040724/PRS_2_A_weight_haufetrans_all.npy") 
weights_act_B = np.load("Haufe_PRS_040724/Weight_Haufe_040724/PRS_2_B_weight_haufetrans_all.npy") 
...
weights_act_sum = weights_act_avg_abs.sum(axis=1) #sum across networks
np.savetxt("weight_maps_HaufeAAB/sig_testing/PRS_2_vertex_SOW_python.csv", weights_act_sum, delimiter=",", header="SOW", comments="")
...
base_dir = "C:/Users/kevin/OneDrive/Documents/NGG_PhD/Alexander-Bloch/F1+F2_Scripts+files/Step_5/FULL/Weight_Haufe_Nulls_041024/"
...
for dir_letter in string.ascii_uppercase[:10]:  # 'A' through 'J'
    dir_path = os.path.join(base_dir, f"Weight_Haufe_Nulls_041024_all_cov_{dir_letter}")
    if not os.path.isdir(dir_path):
        raise FileNotFoundError(f"Directory not found: {dir_path}")
    
    # Loop through files 0.npy … 99.npy
    for i in range(0, 100):
        fn_A = f"PRS_2_A_Weight_haufetrans_all_{i}.npy"
        fp_A = os.path.join(dir_path, fn_A)
        fn_B = f"PRS_2_B_Weight_haufetrans_all_{i}.npy"
...
np.savetxt("weight_maps_HaufeAAB/sig_testing/PRS_2_TFCE_act.csv", tfce_act, delimiter=",", header="tfce_score", comments="")
...
np.savetxt("weight_maps_HaufeAAB/sig_testing/PRS_2_TFCE_p_vals.csv", p_fwe, delimiter=",", header="p_fwe", comments="")
